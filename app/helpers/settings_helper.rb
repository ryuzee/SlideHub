module SettingsHelper
  def generate_form_field(setting, form)
    if %w(site.display_login_link site.only_admin_can_upload site.signup_enabled site.header_inverse).include?(setting.var)
      return form.select :value, [%w(0 0), %w(1 1)], {}, { class: 'form-control' }
    end

    if %w(site.theme).include?(setting.var)
      return form.select :value, [%w(default default), %w(dark dark), %w(white white)], {}, { class: 'form-control' }
    end

    if %w(site.name site.favicon).include?(setting.var)
